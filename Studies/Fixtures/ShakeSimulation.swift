// Frozen experimental solver for shake-response studies. Not part of the app target.
// The app solver remains SmallWave/Core/LiquidSimulation.swift.
import Foundation
import simd
...
final class LiquidSimulation {
    static var diagnosticDefaultForceScale: Float = 5.8
    ...
    private func substep(motion: MotionSample) {
        ...
        for i in particles.indices {
            ...
            if simd_length_squared(particles[i].velocity) > 25 { predictionCaps += 1 }
        ...
        for i in particles.indices {
            let reconstructed = (particles[i].position - particles[i].previous) / dt
            if simd_length_squared(reconstructed) > 25 { constraintCaps += 1 }
            particles[i].velocity = limited(reconstructed, to: 5)
        }
        for i in particles.indices {
